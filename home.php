...
<section class="simplecharm-portfolio-blogs grid lg:grid-cols-3 md:grid-cols-2 sm:grid-cols-1 gap-4 m-3">
		<?php while(have_posts()): ?>
			<?php the_post(); ?>
			<article <?php post_class("simplecharm-portfolio-blog p-2"); ?> id="post-<?php esc_attr(the_ID()); ?>">
				<h2 class="text-2xl"><a href="<?php echo esc_url(get_the_permalink()); ?>" class="underline"><?php echo esc_html(get_the_title()); ?></a></h2>
				<p><?php echo apply_filters('the_excerpt', get_the_excerpt()); ?></p>
				<a href="<?php the_permalink(); ?>" class="bg-blue-500 text-white py-2 px-4 inline-block"><?php _e('Read More',"simplecharm-portfolio"); ?></a>
			</article>
