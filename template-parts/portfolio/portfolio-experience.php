...
if (is_array($args) && array_key_exists("experiences", $args)):
 ...
 <tr class="flex flex-col simplecharm-basic-border simplecharm-basic-padding">
    <td>
        ...
        <input type="text" class="institution" placeholder="Experience Institution" name="simplecharm_portfolio[experiences][<?php echo $key; ?>][][institution]" value="<?php echo (array_key_exists("institution",$flatter_experience)) ? $flatter_experience['institution'] : '' ;?>" id="experience-institution-<?php echo $key; ?>"  data-queue="<?php echo $key; ?>">
    </td>
    <td>
        ...
        <input type="text" class="post-title" placeholder="Post Title" name="simplecharm_portfolio[experiences][<?php echo $key; ?>][][post-title]" value="<?php echo (array_key_exists("post-title",$flatter_experience)) ? $flatter_experience['post-title'] : '' ;?>" id="experience-post-title-<?php echo $key; ?>" data-queue="<?php echo $key; ?>">
    </td>
    <td class="responsibilities" id="repeatable-fieldset-three">
        ...
        <textarea name="simplecharm_portfolio[experiences][<?php echo $key; ?>][][responsibility]" id="responsibilities-<?php echo $key; ?>" cols="50" rows="5" class="responsibility" data-queue="<?php echo $key; ?>"><?php echo (array_key_exists("responsibility",$flatter_experience)) ? $flatter_experience['responsibility'] : '' ;?></textarea>
    </td>
    <td>
        <label for="start-date-<?php echo $key; ?>">Start Date</label>
        <input type="date" class="start_date" name="simplecharm_portfolio[experiences][<?php echo $key; ?>][][start_date]" placeholder="Start Date" id="start-date-<?php echo $key; ?>" data-queue="<?php echo $key; ?>" value="<?php echo (array_key_exists("start_date",$flatter_experience)) ? $flatter_experience['start_date'] : '' ;?>">
    </td>
    <td>
        <label for="end-date-<?php echo $key; ?>">End Date</label>
        <input type="date" class="end_date" name="simplecharm_portfolio[experiences][<?php echo $key; ?>][][end_date]" placeholder="End Date" id="end-date-<?php echo $key; ?>" data-queue="<?php echo $key; ?>" value="<?php echo (array_key_exists("end_date",$flatter_experience)) ? $flatter_experience['end_date'] : '' ;?>" <?php disabled($working_now,'on');?>>
    </td>
    <td>
